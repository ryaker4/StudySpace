...
std::wstring modAlphaCipher::convert(const std::vector<int>& v)
{
    ...
    std::wstring result;
    for(auto i : v) {
        result.push_back(std::toupper(numAlpha[i], loc));
    }
    return result;
}
...
std::wstring modAlphaCipher::getValidKey(const std::wstring & s)
{
    ...
    std::wstring tmp(s);
    for (auto & c:tmp) {
        if (!isalpha(c, std::locale("ru_RU.UTF-8")))
            throw cipher_error("Invalid key");
        if (islower(c, std::locale("ru_RU.UTF-8")))
            c = std::toupper(c, std::locale("ru_RU.UTF-8"));
    }
    return tmp;
}
...
std::wstring modAlphaCipher::getValidOpenText(const std::wstring & s)
{
    ...
    for (auto c:s) {
        if (isalpha(c, std::locale("ru_RU.UTF-8"))) {
            if (islower(c, std::locale("ru_RU.UTF-8")))
                tmp.push_back(std::toupper(c, std::locale("ru_RU.UTF-8")));
            else
                tmp.push_back(c);
        }
    }
...
std::wstring modAlphaCipher::getValidCipherText(const std::wstring & s)
{
    ...
    for (auto c:s) {
        if (!isupper(c, std::locale("ru_RU.UTF-8")))
